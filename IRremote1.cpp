...
void IRsend1::enableIROut(int khz) {
...
void IRrecv1::blink13(int blinkflag)
{
...
int IRrecv1::decode(decode_results *results) {
...
long IRrecv1::decodeNEC(decode_results *results) {
  long data = 0;
...
long IRrecv1::decodeSony(decode_results *results) {
  long data = 0;
...
int IRrecv1::getRClevel(decode_results *results, int *offset, int *used, int t1) {
...
long IRrecv1::decodeRC5(decode_results *results) {
...
long IRrecv1::decodeRC6(decode_results *results) {
...
int IRrecv1::compare(unsigned int oldval, unsigned int newval) {
  if (newval < oldval * .8) {
    return 0;
  } 
 ...
 */
long IRrecv1::decodeHash(decode_results *results) {
...
void IRsend1::sendSharp(unsigned long data, int nbits) {
...
void IRsend1::sendDISH(unsigned long data, int nbits)
{
  enableIROut(56);
  mark(DISH_HDR_MARK);
