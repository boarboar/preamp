...
// NOTE: CHG launchpad with a M430G2552

//#define IR_USE_TIMERA 
#define IR_USE_TIMERA0 
...
#define TIMER_ENABLE_INTR    ({TA1CCTL0 |= CCIE;}) // SH 071112 
...
//#endif
#elif defined(IR_USE_TIMERA0)
#define TIMER_RESET
#define TIMER_ENABLE_PWM     (TA0CTL = TASSEL_2 + MC_1) // SMCLK, Up mode
#define TIMER_DISABLE_PWM    (TA0CTL = TASSEL_2 + MC_0) // SMCLK, Stop mode
#define TIMER_ENABLE_INTR    ({TA0CCTL0 |= CCIE;}) // SH 071112 
#define TIMER_DISABLE_INTR   ({TA0CTL &= ~TAIE; TA0CCTL0 &= ~CCIE;})
#define TIMER_INTR_NAME      TIMER0_A0_VECTOR
#define TIMER_CONFIG_KHZ(val) ({ \
      TA0CCTL0 |= OUTMOD_4;	 \
      TA0CCR0 = FOSC/val/2;      \
})
...
#define TIMER_CONFIG_NORMAL() ({               \
  ...
  TA0CCTL0 |= OUTMOD_4;	                       \
  TA0CCR0 = SYSCLOCK * USECPERTICK / 1000000;                               \
  TA0R = 0;				       \
})
